...
async def reset_servidor(interaction: discord.Interaction):
    ...
    # 1. PEGAR OS CARGOS EXISTENTES
    role_everyone = guild.default_role
    role_mod = discord.utils.get(guild.roles, name="Moderador")
    role_amd_mod = discord.utils.get(guild.roles, name="AMD MOD")
    role_amd = discord.utils.get(guild.roles, name="AMD")
    role_usuario = discord.utils.get(guild.roles, name="Usuário")

    # 2. APAGAR CANAIS ANTIGOS
    ...
    if role_mod:
        ow_filtracao[role_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True)

    cat_filtracao = await guild.create_category("🚨 𝑭𝑰𝑳𝑻𝑹𝑨𝑪𝑨𝑶 ›", overwrites=ow_filtracao)
    ...
    ow_publico_leitura = {
        ...
        role_usuario: discord.PermissionOverwrite(read_messages=True, send_messages=False),
        role_amd: discord.PermissionOverwrite(read_messages=True, send_messages=False),
        role_amd_mod: discord.PermissionOverwrite(read_messages=True, send_messages=False),
    }
    if role_mod:
        ow_publico_leitura[role_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True)

    cat_ing = await guild.create_category("📌 𝑾𝑬𝑳𝑪𝑶𝑚𝑬 ›", overwrites=ow_publico_leitura)
    ...
    # ==========================================
    # 🛒 3. ZYKLON VENDAS (LOJA FF & BOTS)
    # ==========================================
    cat_loja = await guild.create_category("🛒 𝒁𝒀𝑲𝑳𝑶𝑵 𝑽𝑬𝑵𝑫𝑨𝑺 ›", overwrites=ow_publico_leitura)
    ...
    # ==========================================
    # 🛠️ 4. ATENDIMENTO & TICKETS (PÚBLICO)
    # ==========================================
    ...
    cat_ticket = await guild.create_category("🛠️ 𝑨𝑻𝑬𝑵𝑫𝑰𝑑𝑬𝑵𝑻𝑶 ›", overwrites=ow_publico_interativo)
    ...
    if role_amd_mod:
        ow_amostradinhos[role_amd_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True, connect=True)
    if role_mod:
        ow_amostradinhos[role_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True, connect=True)

    cat_ff = await guild.create_category("🏆 𝑶𝑺 𝑨𝑴𝑶𝑺𝑻𝑑𝑨𝑫𝑰𝑵𝑯𝑶𝑺 ›", overwrites=ow_amostradinhos)
    ...
    if role_amd_mod:
        ow_so_amd[role_amd_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True)
    if role_mod:
        ow_so_amd[role_mod] = discord.PermissionOverwrite(read_messages=True, send_messages=True)

    await guild.create_text_channel("🔒・chat-lideres-amd", category=cat_ff, overwrites=ow_so_amd)
    ...
    cat_admin = await guild.create_category("🔒 𝑨𝑫𝑑𝑰𝑵 𝑷𝑨𝑵𝑬𝑳 ›", overwrites=ow_admin)
